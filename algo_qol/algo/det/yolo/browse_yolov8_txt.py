...
def draw_box_on_image(img_path, label_path, classes, colors, output_f, valid_class_index=None,
                      draw_bbox=True):
    """
    ...
    save_file_path = os.path.join(
        output_f, image_name)  # 本次保存图片jpg路径

    # flag_people_or_car_data = 0  #变量 代表类别
    source_file = open(label_path) if os.path.exists(label_path) else []
...
def yolo_dataset_visualization(
        img_f,
        label_f,
        output_f,
        class_txt_path,
    ...
    img_list = get_file_recursively(img_f)
    img_list = list(filter(lambda x: x.endswith('.jpg') or x.endswith('.png') or x.endswith('.jpeg'), img_list))
    ...
    for img_path in img_list:
        # get relative path to img_f
        image_name = os.path.relpath(img_path, img_f)
        label_path = os.path.join(label_f, os.path.splitext(image_name)[0] + '.txt')

        box_num = draw_box_on_image(
            img_path, label_path, classes, colors, output_f, valid_class_index=valid_class_index,
...
if __name__ == '__main__':
    ...
    Fire(yolo_dataset_visualization)
