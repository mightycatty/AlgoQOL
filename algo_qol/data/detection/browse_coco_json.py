...
def show_coco_json(args):
    ...
    for i in range(len(image_ids)):
        ...
        if args.save_f is not None:
            os.makedirs(args.save_f, exist_ok=True)
            dst = os.path.join(args.save_f, os.path.basename(image_data['file_name']))
            plt.savefig(dst)
        else:
            if args.wait_time == 0:
                plt.show()
...
def crop_samples(ann_file, category_name='all', num=10, save_f='./crop_results'):
    coco = COCO(ann_file)
    print(f'Total number of images：{len(coco.getImgIds())}')
    categories = coco.loadCats(coco.getCatIds())
    category_names = [category['name'] for category in categories]
    ...
    for category_name in category_names:
        print('cropping {} images'.format(category_name))
        category_ids = coco.getCatIds(category_name)
        image_ids = coco.getImgIds(catIds=category_ids)
        np.random.shuffle(image_ids)
        sub_save_f = os.path.join(save_f, category_name)
        os.makedirs(sub_save_f, exist_ok=True)
        for i in range(len(image_ids)):
            image_data = coco.loadImgs(image_ids[i])[0]
            ...
            image = image[bbox_y:bbox_y + bbox_h, bbox_x:bbox_x + bbox_w]
            cv2.imwrite(osp.join(sub_save_f, os.path.basename(image_path)), image)
            if i >= num:
                break
...
def parse_args():
    ...
    parser.add_argument(
        '--img-dir', default='/mnt/juicefs/cv_tikv/heshuai/datasets/openbrand/images', help='image folder path')
    parser.add_argument(
        '--ann-file',
        default='/mnt/juicefs/cv_tikv/heshuai/datasets/openbrand/openbrand_train.json',
        help='ann file path')
    parser.add_argument(
    ...
    parser.add_argument('--save_f', type=str, default='coco_vis')
